...
static const std::vector<OperatorGeneratorArgs> opGenArgs{
    ...
    OperatorGeneratorArgs(
        ...
        aliasAnalysisFromSchema()),
    DEFINE_UNARY_OP_WITH_COMPLEX(aten::log, ::log(a), float, float),
...
#define DEFINE_CONVERT_BASE_OP(op_name, prefix, char_op) \
  OperatorGeneratorArgs(                                 \
    ...
    DEFINE_GENERIC_BINARY_OP(
        ...
        float,
        complex),
    DEFINE_INT_FLOAT_OP(aten::log, ::log(a) / ::log(b), float),
    DEFINE_INT_COMPLEX_OP(aten::log, ::log(a) / ::log(b), complex),
    DEFINE_FLOAT_COMPLEX_OP(aten::log, ::log(a) / ::log(b), complex),
    DEFINE_SCALAR_BINARY_OP_AVOID_COLLISION(
        aten::log,
        ::log(a) / ::log(b),
        ::log(a) / ::log(b),
        float),
    DEFINE_UNARY_OP(aten::log1p, ::log1p(a), float, float),
    DEFINE_UNARY_OP_WITH_COMPLEX(aten::log10, ::log10(a), float, float),
